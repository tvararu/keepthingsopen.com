...
```sh
mise build       # Render dist/index.html from index.html + signatures.csv
...
1. Reads `signatures.csv`, parses it with the built-in `parseCSV`, drops the header row, sorts the rows case-insensitively by name, and treats each row with a name as one signature.
2. Parses each row into `name, role, org, contributor` columns via `parseRow`. Only `name` and the `contributor` flag (the literal string `true` in column 4) are used by the renderer; `role` and `org` are currently ignored. Rows without a name are silently skipped.
3. Renders named signatures to `<div class="sig">…</div>` cards via `renderCard` (with `escapeHtml` on the name), and collapses all `Anonymous` rows into a single counted group via `renderAnonymousGroup`.
...
If you change the column format that the form emits, change `parseRow` to match — the build-time parser and the runtime form-builder share a column order but no code.
...
- `signatures.csv` — source of truth for signatures, one CSV row per signature with header `name,role,org,contributor`. The build sorts rows case-insensitively by name before rendering; `parseRow` skips rows without a name silently.
